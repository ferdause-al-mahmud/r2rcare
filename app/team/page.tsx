      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
        {teamMembers.map((member, index) => {
          ...
          return (
            <div
              key={index}
              className="relative cursor-pointer group overflow-hidden rounded-2xl shadow-sm transition-transform duration-300 hover:-translate-y-1 bg-[#f9f9f9]"
            ...
            >
              <div className="relative w-full h-[420px] overflow-hidden rounded-2xl">
                ...
                {/* Purple Overlay */}
                <div
                  className={`absolute inset-0 bg-[#77429AD9] m-6 border border-[#FFFFFF6B] rounded-2xl transition-opacity duration-300 flex flex-col items-center justify-center text-center px-6 ${
                    isMobile
                      ? isActive
                        ? "opacity-100"
                        : "opacity-0"
                ...
                {/* Floating Info Box */}
                <div
                  className={`absolute left-1/2 -translate-x-1/2 bottom-5 bg-white rounded-xl shadow-md py-3 px-5 w-[85%] transition-all duration-300 ${
                    isMobile && isActive ? "opacity-0" : "group-hover:opacity-0"
                  }`}
                >
